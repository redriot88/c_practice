...
int main() {
  ...
  //create race cars with int total lap times
  struct RaceCar Mick = {"Michael Schumacher", "red Ferrari", 0};
  struct RaceCar Louis = {"Louis Hamilton", "black Mercedes", 0};
  struct RaceCar Oscar = {"Oscar Piastri", "orange McLaren", 0};
  struct RaceCar Max = {"Max Verstappen", "blue Redbull", 0};
  ...
  updateFirstPlace(&grandPrix, &Mick, &Louis, &Oscar, &Max);
  printFirstPlaceAfterLap(grandPrix);
  }

  //find winner with lowest total time
struct RaceCar* winner = &Mick;
if (Louis.totalLapTime < winner->totalLapTime) winner = &Louis;
if (Oscar.totalLapTime < winner->totalLapTime) winner = &Oscar;
if (Max.totalLapTime < winner->totalLapTime) winner = &Max;
